...
function planLabel(value: string): string {
  ...
  if (value === "control_tower") {
    return t.plans.controlTower;
  }
...
function planDescription(value: string): string {
  if (value === "starter") {
    return t.plans.starterDesc;
  }
  if (value === "growth") {
    return t.plans.growthDesc;
  }
  if (value === "control_tower") {
    return t.plans.controlTowerDesc;
  }
...
function reasonLabel(reason?: UsageBlockReason): string {
  ...
  if (reason === "PDF_NOT_AVAILABLE_IN_PLAN") {
...
function policyStatusLabel(policies: ProjectPolicies): "Aktywne" | "Wyłączone" | "Limit osiągnięty" {
  ...
  if (policies.ai.usageThisMonth >= policies.ai.monthlyLimit) {
...
export default function ProjectPoliciesSettingsPage() {
  const params = useParams<{ workspaceSlug: string }>();
  const workspaceSlug = typeof params.workspaceSlug === "string" ? params.workspaceSlug : "workspace";
  ...
  const visibleEntries = useMemo<PolicyAuditEntry[]>(() => {
    if (showFullHistory) {
      return [...policies.policyAudit].reverse();
    }
